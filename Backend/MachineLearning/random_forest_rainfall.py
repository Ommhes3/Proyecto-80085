...
def prepare_dataset(df_wide: pd.DataFrame):
    """
    Prepara X e y para predecir precipitación en arroz.

    y = rainfall

    X = temperature, humidity, solar_radiation, soil_ph, wind_speed,
        zone, condition
    """

    df = df_wide.copy()

    # Solo se predice lluvia para arroz
    df = df[df["crop"] == "rice"]

    if df.empty:
        raise ValueError("No hay registros de arroz para entrenar el modelo de rainfall.")
    ...
    # Se eliminan filas sin rainfall, porque es la variable objetivo
    ...
    df = df.dropna(subset=["zone", "condition"])

    if len(df) < 10:
        raise ValueError(
            f"Hay muy pocos registros para entrenar el modelo de rainfall. Registros disponibles: {len(df)}"
        )
...
def evaluate_model(y_test, y_pred):
    ...
    results = pd.DataFrame({
        "rainfall_real": y_test.values,
        "rainfall_predicted": y_pred
    })
    ...
    print("\nRESULTADOS DEL MODELO RANDOM FOREST - RAINFALL RICE")
    print("-" * 60)
    print(f"MAE:  {mae:.4f}")
...
def main():
    ...
    print("\nPreparando variables para predecir rainfall en arroz...")
    ...
    df_model.to_csv(output_dir / "rainfall_rice_ml_dataset_from_influx.csv", index=False)
    results.to_csv(output_dir / "rainfall_rice_predictions.csv", index=False)
    metrics.to_csv(output_dir / "rainfall_rice_metrics.csv", index=False)
    importance.to_csv(output_dir / "rainfall_rice_feature_importance.csv", index=False)

    versioned_model_path = output_dir / f"random_forest_rainfall_rice_model_{timestamp}.joblib"
    latest_model_path = output_dir / "random_forest_rainfall_rice_model_latest.joblib"

    joblib.dump(model, versioned_model_path)
    joblib.dump(model, latest_model_path)

    print("\nArchivos generados:")
    print(output_dir / "rainfall_rice_ml_dataset_from_influx.csv")
    print(output_dir / "rainfall_rice_predictions.csv")
    print(output_dir / "rainfall_rice_metrics.csv")
    print(output_dir / "rainfall_rice_feature_importance.csv")
